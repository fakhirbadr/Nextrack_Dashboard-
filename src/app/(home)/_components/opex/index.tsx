...
import res from "@/app/data/opex.json";
const data = res.data;

const budget = res.budget;
const actual = res.actual;
const percentage = Math.round(((9000 - actual) / 9000) * 100);

// eslint-disable-next-line @typescript-eslint/no-empty-object-type
interface CapexChartProps extends React.HTMLAttributes<HTMLDivElement> {}
export default function OpexWidget({ className, ...props }: CapexChartProps) {
  return (
    <Card className={cn("flex flex-col", className)} {...props}>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 p-2">
        <CardTitle className="text-xl font-medium text-white/70">
          Suivi des Objectifs de Maintenance corrective
        </CardTitle>
        <div className="text-right">
          <div className="text-2xl font-bold">{actual.toLocaleString()} PV</div>
